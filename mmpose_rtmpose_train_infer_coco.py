...
class Config:
    ...
    DATA_DIR = os.path.join(BASE_DIR, "allframes-annotation")  # COCO dataset root
    ...
    # COCO JSON annotation paths
    ANN_DIR = os.path.join(DATA_DIR, "annotations")
    COCO_JSON = os.path.join(ANN_DIR, "person_keypoints_default.json")
    ...
    # Keypoint names (matching COCO JSON)
    ...
    def setup(cls):
        ...
        print(f"  Dataset dir: {cls.DATA_DIR}")
        print(f"  COCO JSON  : {cls.COCO_JSON}")
...
# =============================================
# 3. COCO JSON Annotation Loading & Splitting
# =============================================
def load_and_split_coco_json() -> Dict[str, str]:
    """
    Load the pre-existing COCO JSON annotations and split into
    train/val/test subsets.

    The source JSON is expected to be in standard COCO keypoint format
    with categories, images, and annotations.

    Returns dict of split-name → JSON file path.
    """
    import random

    print("\n" + "-" * 60)
    print("STEP 1: Loading COCO JSON annotations & splitting")
    print("-" * 60)

    if not os.path.exists(Config.COCO_JSON):
        raise FileNotFoundError(
            f"COCO JSON not found at: {Config.COCO_JSON}\n"
            f"Expected location: {Config.ANN_DIR}/person_keypoints_default.json")
    ...
    image_indices = list(range(num_total))
    rng.shuffle(image_indices)

    # Compute split sizes
    n_train = int(num_total * Config.TRAIN_RATIO)
    n_val = int(num_total * Config.VAL_RATIO)
    # Rest goes to test
    ...
    for split_name, indices in splits.items():
        split_images = [all_images[i] for i in indices]
        ...
        # Re-number annotation IDs to be contiguous within split
        for new_id, ann in enumerate(split_annotations, start=1):
            ann = dict(ann)  # copy to avoid mutating original
            split_annotations[new_id - 1] = ann

        # Build split JSON (preserving original COCO structure)
        split_coco = {
        ...
        out_json = os.path.join(Config.ANN_DIR, f"{split_name}.json")
        with open(out_json, 'w') as f:
            json.dump(split_coco, f)

        json_paths[split_name] = out_json
        print(f"  {split_name:5s}: {len(split_images):5d} images, "
              f"{len(split_annotations):5d} annotations → {out_json}")

    print(f"\n  Split ratio: "
          f"train={Config.TRAIN_RATIO:.0%} / "
...
def build_mmpose_config(coco_json_paths: Dict[str, str]):
    ...
    custom_metainfo = dict(
        ...
        keypoint_info={
            i: dict(name=Config.KEYPOINT_NAMES[i], id=i, color=[255, 0, 0],
                    type='', swap='')
    ...
    # === Dataset overrides ===
    # All splits share the same data_root (dataset root) and images folder.
    # ann_file paths are absolute paths to the split JSON files.
    ...
    cfg.train_dataloader.dataset.data_root = Config.DATA_DIR
    ...
    cfg.val_dataloader.dataset.data_root = Config.DATA_DIR
    ...
    print(f"     - Dataset: COCO standard → custom paper_corners")
...
class InferenceEngine:
    ...
    def benchmark(self, test_dir: str, coco_json_path: str,
        ...
        # test_dir is the images directory directly
        images_dir = test_dir
...
def main():
    ...
    # Step 1: Load & split COCO JSON annotations
    coco_json_paths = load_and_split_coco_json()
    ...
    metrics = engine.benchmark(
        Config.IMAGES_DIR, test_json, save_visualizations=True)
