...
class Config:
    ...
    def setup(cls):
        if not os.path.exists("/kaggle"):
            ...
            cls.DATA_DIR = os.path.join(cls.BASE_DIR, "keypointframes")
            cls.OUTPUT_DIR = os.path.join(cls.BASE_DIR, "outputs")
            cls.VIS_DIR = os.path.join(cls.OUTPUT_DIR, "visualizations")
            cls.TRAIN_DIR = cls.DATA_DIR
            cls.VAL_DIR = cls.DATA_DIR
            cls.TEST_DIR = cls.DATA_DIR
        else:
            print("Running on Kaggle")
            cls.BASE_DIR = "/kaggle/working"
            cls.DATA_DIR = "/kaggle/input/datasets/sounakp/ketpoints/keypointframes"
            cls.OUTPUT_DIR = os.path.join(cls.BASE_DIR, "outputs")
            cls.VIS_DIR = os.path.join(cls.OUTPUT_DIR, "visualizations")
            cls.TRAIN_DIR = cls.DATA_DIR
            cls.VAL_DIR = cls.DATA_DIR
...
class KeypointDataset(Dataset):
    def __init__(self, data_dir: str, input_size: int = 256, num_keypoints: int = 4):
        ...
        annotation_file_coco = os.path.join(data_dir, "_annotations.coco.json")
        annotation_file_cvat = os.path.join(data_dir, "annotations", "person_keypoints_default.json")
        
        if os.path.exists(annotation_file_cvat):
            annotation_file = annotation_file_cvat
            self.image_dir = os.path.join(data_dir, "images")
        elif os.path.exists(annotation_file_coco):
            annotation_file = annotation_file_coco
            self.image_dir = data_dir
        else:
            raise FileNotFoundError(f"Annotation file not found in {data_dir}")
