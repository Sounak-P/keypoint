...
class TwoStageConfig:
    ...
    RFDETR_WEIGHTS: str = r"C:\TCS\keypoint\toTest\weights\checkpoint_best_total.pth"
    KP_WEIGHTS: str = r"C:\TCS\keypoint\toTest\weights\final_model.ckpt"
    ...
    # Anti-flicker for stationary objects
    HEATMAP_BLUR_SIGMA: float = 1.0       # Gaussian blur on heatmap before peak finding
    SNAP_PIXEL_THRESHOLD: float = 3.0     # px in frame space; below this → keep smoothed pos
    ADAPTIVE_ALPHA_SCALE: float = 25.0    # displacement (px) at which full EMA_ALPHA kicks in
    MIN_ALPHA: float = 0.08               # floor for adaptive alpha (heaviest smoothing)
...
def load_keypoint_model(ckpt_path: str, device: str,
    ...
    # NOTE: torch.compile with inductor backend requires a C++ compiler
    # (cl.exe on Windows / gcc on Linux).  On machines without one the
    # compile step crashes at runtime.  For live-cam inference the JIT
    # overhead actually hurts latency, so we skip it entirely.
    # If you want to re-enable it, uncomment the block below.
    ...
    print("[SE-UNet] torch.compile() skipped (not needed for live inference)")
...
def remap_keypoints(kps_in_crop, meta: Dict) -> List[Optional[Tuple[float, float, float]]]:
    """
    Map keypoints from crop-space coordinates back to full-frame coordinates.
    Returns float coordinates to preserve sub-pixel precision through the
    smoothing pipeline — final int conversion happens at draw time.

    kps_in_crop : list of (cx, cy, conf) or None — in [0..KP_INPUT_SIZE]
    Returns     : list of (fx, fy, conf) or None — in full-frame pixels (float)
    """
    ...
    for kp in kps_in_crop:
        if kp is None:
            ...
            fx = kp[0] * sx + cx0     # keep float — no int() truncation
            fy = kp[1] * sy + cy0
...
def extract_keypoints_from_heatmap(
    ...
    Applies Gaussian blur to stabilise peaks, then sub-pixel quadratic
    refinement around the argmax for ~0.1 px precision.
    """
    results = []
    for ch in range(hm.shape[0]):
        channel = hm[ch]

        # ── Gentle Gaussian blur to suppress sensor / quantisation noise
        if blur_sigma > 0:
            channel = gaussian_filter(channel, sigma=blur_sigma)
        ...
        # Fit a 1-D parabola along each axis using the 3 neighbouring
        # values and solve for the fractional offset of the true peak.
        x_sub, y_sub = float(x), float(y)
        H, W = channel.shape
        if 1 <= y <= H - 2:
            vm = channel[y - 1, x]
            v0 = channel[y,     x]
            vp = channel[y + 1, x]
            denom = 2.0 * (2.0 * v0 - vm - vp)
            if abs(denom) > 1e-6:
                y_sub = y + (vm - vp) / denom
        if 1 <= x <= W - 2:
            vm = channel[y, x - 1]
        ...
        results.append((x_sub, y_sub, float(vals[idx])))
    return results
...
class ObjectTracker:
    """
    Tracks keypoints for a single detected object across frames using
    **adaptive** EMA + dead-zone snapping.

    When a keypoint barely moves (stationary object), the alpha drops
    towards MIN_ALPHA or the position is snapped entirely, eliminating
    flicker.  When it moves significantly (object in motion), the full
    alpha is used so tracking stays responsive.
    """
    def __init__(self, n_kp: int = 4, alpha: float = 0.55, max_missing: int = 4,
                 snap_px: float = 3.0, alpha_scale: float = 25.0,
                 min_alpha: float = 0.08):
        self.n_kp        = n_kp
        self.alpha       = alpha
        self.max_missing = max_missing
        self.snap_px     = snap_px      # dead-zone radius in frame pixels
        self.alpha_scale = alpha_scale   # displacement at which full alpha is used
        self.min_alpha   = min_alpha
        self.smooth: List[Optional[Tuple[float, float, float]]] = [None] * n_kp
        self.missing:  List[int] = [0] * n_kp

    def update(self, raw: List[Optional[Tuple[float, float, float]]]):
        out, vis = [], []
        for i, kp in enumerate(raw):
            if kp is not None:
                ...
                if self.smooth[i] is None:
                    ...
                    dx = kp[0] - self.smooth[i][0]
                    dy = kp[1] - self.smooth[i][1]
                    disp = math.sqrt(dx * dx + dy * dy)

                    if disp < self.snap_px:
                        # ── Dead-zone: keypoint barely moved → snap
                        # Keep existing position, only gently update conf
                        sc = self.min_alpha * kp[2] + (1 - self.min_alpha) * self.smooth[i][2]
                        self.smooth[i] = (self.smooth[i][0], self.smooth[i][1], sc)
                    else:
                        # ── Adaptive alpha: ramp from min_alpha → alpha
                        # based on how far the keypoint actually moved
                        t = min(1.0, disp / self.alpha_scale)
                        a = self.min_alpha + (self.alpha - self.min_alpha) * t
                        sx = a * kp[0] + (1 - a) * self.smooth[i][0]
                        sy = a * kp[1] + (1 - a) * self.smooth[i][1]
                        sc = a * kp[2] + (1 - a) * self.smooth[i][2]
                        self.smooth[i] = (sx, sy, sc)

                out.append((int(round(self.smooth[i][0])),
                            int(round(self.smooth[i][1])),
                            self.smooth[i][2]))
                vis.append(True)
            else:
                self.missing[i] += 1
        ...
        for ni, nb in enumerate(new_boxes):
            ...
            if eid is None:
                ...
                self._trackers[eid] = ObjectTracker(
                    ...
                    snap_px=TwoStageConfig.SNAP_PIXEL_THRESHOLD,
                    alpha_scale=TwoStageConfig.ADAPTIVE_ALPHA_SCALE,
                    min_alpha=TwoStageConfig.MIN_ALPHA,
                )
...
def main():
    ...
    ap.add_argument("--output", default=r"C:\TCS\keypoint\output_two_stage_result.mp4",
        ...
        if last_detections is not None and len(last_detections) > 0:
            ...
            for hm, meta in zip(heatmaps_list, crops_meta):
                kps_crop  = extract_keypoints_from_heatmap(
                    ...
                    TwoStageConfig.HEATMAP_BLUR_SIGMA,
                )
